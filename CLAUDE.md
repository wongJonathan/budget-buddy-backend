...
- **`app/services/savings.py:balance` counts only the Transaction's own `deleted_at`**, never an ancestor's: the ancestor rule is right for listing what a user can see and wrong for counting what a user has. Under ADR 0014, reads include deleted rows by default, so money sums must filter the flag explicitly rather than trusting a read helper.
...
- Soft-delete: Budget, Expense, Transaction, Savings and Category extend `CreatableModel` (`app/database.py`), and a row is deleted exactly when `deleted_at IS NOT NULL`. `DELETE` routes set `deleted_at = func.now()`, never a Python `datetime`: `now()` is fixed per DB transaction, which is what lets ADR 0014's Restore match withdrawn rows by equal `deleted_at`. `updated_at` is kept by `onupdate=func.now()` (SQLAlchemy-side, so raw SQL doesn't bump it), and `eager_defaults` on `CreatableModel` fetches server values back via `RETURNING` — without it, reading them after a write raises `MissingGreenlet` under the async session. Only `User` hard-deletes. A deleted Category is gone from lists, lookups and new rows (`live_categories`), but Expenses already carrying it keep it. See ADR 0007. Per ADR 0014 (not yet implemented), Transaction and Expense reads include deleted rows by default (`include_deleted=false` hides them). Deleted rows are read-only, count toward nothing, and are never valid parents.
- **The ancestor rule runs Budget → Expense and stops** (ADR 0007 as amended by 0011). Whether a Transaction counts depends only on its own flag, so *hiding* through an ancestor never unspends anything. **Deleting an Expense is different: it explicitly withdraws its Period's Transactions** (ADR 0014, not yet implemented). In one DB transaction it soft-deletes every Transaction with that `expense_id`, drains the Savings remainder to the Pool as a `TRANSFER` pair, closes the Savings, and deletes the Expense. **Restore** reverses exactly that within the same Period. It finds the withdrawn Transactions by `deleted_at` equal to the Expense's, which only works because the delete is a single DB transaction and withdrawn rows are read-only.
- **Two guards the model depends on** (ADR 0011): an Expense outside the current Period cannot be edited or deleted (`_require_open_period`), and the active Budget cannot be deleted. Deleting the current-Period Expense of a funded lineage drains its fund back to the Pool as a `TRANSFER` pair and closes the fund — never as `INCOME`, which would assert money arrived from outside (ADR 0014 re-confirms this).
