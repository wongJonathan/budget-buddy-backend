...
- `Expense` is the single row type covering planned spending, savings goals and debt paydown — there's no separate SavingFund or BudgetItem table (ADR 0002, superseded in part by 0009).
...
- **Rollover and Activation are not implemented yet** — only their schema groundwork (`period`, `series_id`, the unique constraint, and the `rolled_periods` table) exists. The routes currently in place are plain CRUD with no walk-forward or copy-on-activate logic wired up. When Rollover is written: it runs as a scheduled job, an Expense is *met* when its Transactions reach its `monthly_cost`, a carried-forward Expense is an ordinary Expense with `frequency = once` and no marker, and idempotency comes from `rolled_periods` — never from checking whether the target period looks empty (ADR 0010).
- Soft-delete: `Budget.is_deleted`, `Expense.is_deleted` and `Transaction.is_deleted` — their `DELETE` routes set the flag (see `app/services/`), and list queries filter it out through `app/services/visibility.py`. `User` and `Category` hard-delete. See ADR 0007.
